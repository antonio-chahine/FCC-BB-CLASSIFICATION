...
if args.compare:
    ...
    outdir = "Classification_AB/compare"
    os.makedirs(outdir, exist_ok=True)
    random.seed(42)
    ...
    # Balanced dataset: same signal as background
    sampled_signal = random.sample(all_signal, len(all_background))

    # -----------------------------
    #  Extract features (same X,y used for both models)
    # -----------------------------
    X, y = get_features_and_labels(sampled_signal, all_background)
    ...
    y_proba_1 = clf1.predict_proba(X_test)[:, 1]

    # =================================================
    # ============ TRAIN MODEL 2 (OPTIMISTIC) ==========
    # =================================================
    print("\n========== TRAINING MODEL 2 (analytic Δz) ==========")
    ...
    y_proba_2 = clf2.predict_proba(X_test)[:, 1]

    # =================================================
    # ============== AUC + DeLong ======================
    # =================================================
    auc1 = roc_auc_score(y_test, y_proba_1)
    auc2 = roc_auc_score(y_test, y_proba_2)
    delta_auc = auc1 - auc2

    print("\n========== ROC-AUC COMPARISON ==========")
    print(f"Model 1 (baseline) AUC = {auc1:.5f}")
    print(f"Model 2 (analytic) AUC = {auc2:.5f}")
    print(f"ΔAUC = {delta_auc:.5f}")

    p_value = delong_roc_test(y_test, y_proba_1, y_proba_2)

    print(f"DeLong p-value = {p_value:.5f}")
    if p_value < 0.05:
        print("➡️ Difference is statistically significant.")
    else:
        print("➡️ No significant difference detected.")

    # Save numbers
    with open(os.path.join(outdir, "delong_results.txt"), "w") as f:
        f.write(f"AUC1 = {auc1:.5f}\n")
        f.write(f"AUC2 = {auc2:.5f}\n")
        f.write(f"ΔAUC = {delta_auc:.5f}\n")
    ...
    plt.plot(fpr2, tpr2, label=f"Optimistic (AUC={auc2:.3f})")
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig(os.path.join(outdir, "ROC_compare.png"))
    plt.close()
    ...
    for idx, (pred, model_name) in enumerate(
            [(y_pred_1, "baseline"), (y_pred_2, "optimistic")]):
        ConfusionMatrixDisplay.from_predictions(
            y_test, pred, display_labels=["Background", "Signal"],
