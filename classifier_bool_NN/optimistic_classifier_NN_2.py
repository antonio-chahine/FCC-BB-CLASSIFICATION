...
if args.classify:
    ...
    X_np, y_np = get_features_and_labels(all_signal, all_background)
    ...
    def train_epoch(model, optimizer):
        ...
        for X_train, y_train in dloader_train:
            ...
            loss = loss_fcn(y_pred, y_train.view(-1, 1))
        ...
        with torch.no_grad():
            for X_valid, y_valid in dloader_validate:
                ...
                vloss = loss_fcn(y_pred_v, y_valid.view(-1, 1))
    ...
    tloss, vloss = [], []
    torch.manual_seed(1)
    for epoch in trange(200, desc="Training epochs"):
...
if args.evaluate:
    ...
    test_accuracy = (pred_labels == y_test).float().mean()
    print(f"Test accuracy = {test_accuracy.item()*100:.1f}%")

    # --- ROC CURVE ---
    y_true = y_test.cpu()
    y_true_np = y_test.numpy().reshape(-1)
