...
parser.add_argument("--data", default="muons_energycut_nomultiplcity_v2_patches_32size_0.0125.npz")
...
if args.classify:
    ...
    outdir = "CNN_muons_energycut_nomultiplcity_v2_patches_32size_0.0125"
...
if args.evaluate:
    ...
    print("Signal eff:", tpr[idx])
    print("Background rejection:", 1 - fpr[idx])


    sig = probs[labels == 1]
    bkg = probs[labels == 0]

    plt.hist(sig, bins=50, alpha=0.5, density=True, label="Signal")
    plt.hist(bkg, bins=50, alpha=0.5, density=True, label="Background")
